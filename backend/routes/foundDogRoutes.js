...
router.post("/founddog", async (req, res) => {
  ...
  if (!breed || !size || !gender || !location) {
    return res.status(400).json({
      message: "All fields except 'details' are required.",
    });
  }
  ...
  try {
    ...
    if (file) {
      ...
      const fullPath = path.join(__dirname, "../../uploads/foundDogs", filename);
    ...
    console.error("Error creating found dog profile:", error);
    res.status(500).json({
